...
fn scan(path: &Path, root: &Path) -> Result<Vec<FileNode>, String> {
    ...
    entries.into_iter().filter(|entry| {
        ...
        Ok(FileNode {
            ...
            directory,
            children: if directory { scan(&path, root)? } else { Vec::new() },
        })
    }).collect()
}

fn root(state: &State<'_, Workspace>) -> Result<PathBuf, String> {
    state.0.lock().map_err(|_| "workspace lock is poisoned".to_string())?
        .clone().ok_or_else(|| "no workspace is open".to_string())
}
...
fn safe_existing(root: &Path, relative: &str) -> Result<PathBuf, String> {
    ...
    if candidate.starts_with(root) { Ok(candidate) } else { Err("path escapes the workspace".into()) }
}
...
#[tauri::command]
fn save_workspace_file(path: String, contents: String, state: State<'_, Workspace>) -> Result<(), String> {
    let root = root(&state)?;
    let path = safe_existing(&root, &path)?;
    if !path.is_file() { return Err("only existing workspace files can be saved".into()); }
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![choose_workspace, reopen_workspace, list_workspace, read_workspace_file, save_workspace_file])
